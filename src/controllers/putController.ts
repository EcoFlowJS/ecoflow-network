...
async function putController(this: EcoContext) {
  const { _ } = ecoFlow;
  ...
  const { urlEndpoint, bupassValue, putValue } = inputs;
  ...
  const axiosPutValue = bupassValue
    ? !_.isUndefined(value) && !_.isEmpty(value) && _.isObjectLike(value)
      ? value
      : {}
    : !_.isUndefined(putValue) &&
      !_.isEmpty(putValue) &&
      putValue.validate &&
      _.isString(putValue.value)
    ? JSON.parse(putValue.value)
    : {};
  ...
  try {
    payload.msg = (await axios.put(parsedURI, axiosPutValue)).data;
    next();
  } catch (error) {
    payload.msg = error;
    next();
